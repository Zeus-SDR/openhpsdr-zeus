...
| 0x0b | [22] | CW keyer reverse (swap dot/dash paddles) |
| 0x0b | [15:14] | CW keyer mode (0=straight/external/bug, 1=Mode A, 2=Mode B) |
| 0x0b | [13:8] | CW keyer speed in WPM (0-60) |
| 0x0b | [7] | CW keyer letter spacing (0=off, 1=on) |
| 0x0b | [6:0] | CW keyer weight (33-66) |
...
> **Zeus note (zeus-bks):** the "There is no internal keyer" sentence above
> is stale upstream text and contradicts its own next clause. Current HL2
> gateware *does* ship an internal iambic keyer (`rtl/cw_openhpsdr.sv` +
> `rtl/iambic.v`): with a paddle on the KEY jack the FPGA generates the
> dit/dah timing and the shaped CW envelope. The host configures it via
> register **0x0B** (see the memory map above) — keyer speed (WPM), mode
> (straight / iambic A / iambic B), weight, letter-spacing and paddle
> reverse. In *straight* mode the gateware passes the key line straight
> through and ignores keyer speed, matching the legacy behaviour the
> sentence describes. Zeus writes 0x0B from the CW panel; see
> `Zeus.Protocol1/ControlFrame.cs` (`CwKeyerConfig`).
